        ...
        if (option.hasQuantity) {
            quantityHtml = `
                <div class="quantity-controls">
                    <button type="butto,," class="quantity-btn minus">-</button>
        ...
        card.innerHTML = `
            <div class="option-content">
                ...
                <div class="option-footer">
                    ...
                    <button type="button" class="toggle-btn">Ajouter</button>
                </div>
            </div>
        `;
...
document.addEventListener('DOMContentLoaded', () => {
    const hiddenInput = document.getElementById('selectedInputOptions');

    // Récupérer ton gestionnaire d'options déjà existant
    const manager = new OptionsManager();

    // Fonction pour mettre à jour la valeur de l'input caché
    function updateSelectedOptionsInput() {
        const selectedArray = Array.from(manager.selectedOptions);
        const optionsSummary = selectedArray.map(optionId => {
            const option = options.find(opt => opt.id === optionId);
            const quantity = manager.quantities.get(optionId) || 1;
            return `${option.name} (x${quantity})`;
        });
        hiddenInput.value = optionsSummary.join('  |  ');
    }

    // Intercepter les ajouts/suppressions d'options
    document.getElementById('optionsGrid').addEventListener('click', (e) => {
        if (e.target.closest('.toggle-btn')) {
            setTimeout(updateSelectedOptionsInput, 50); // Laisse le temps à OptionsManager de mettre à jour
        }
    });
});
